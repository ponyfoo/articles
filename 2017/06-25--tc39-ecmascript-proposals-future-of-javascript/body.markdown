...
In our example, the response object is what we're after, so we'll flip things over and assigned the result of `await fetch` to the `response` variable, instead of using a promise reaction.
